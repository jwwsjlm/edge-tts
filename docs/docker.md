...
发布镜像：`ghcr.io/jwwsjlm/edge-tts:7.3.1`，支持 `linux/amd64` 和 `linux/arm64`。服务器建议固定版本，不要长期依赖 `latest`。离线 Release 资产仅提供 `linux/amd64`。
...
```bash
printf 'EDGE_TTS_IMAGE_TAG=7.3.1\n' > .env
...
docker image inspect ghcr.io/jwwsjlm/edge-tts:7.3.1 >/dev/null
```
